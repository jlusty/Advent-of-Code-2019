...
fn get_pos(wire: Vec<(char, isize)>) -> Vec<(isize, isize)> {
    let mut pos_vec: Vec<(isize, isize)> = Vec::new();
    pos_vec.push((0, 0));
    ...
    for i in wire.into_iter() {
        match i.0 {
            'U' => {
                for y in 1..i.1 + 1 {
                    pos_vec.push((current_pos.0, current_pos.1 + y));
                }
            ...
            'L' => {
                for x in 1..i.1 + 1 {
                    pos_vec.push((current_pos.0 - x, current_pos.1));
                }
                current_pos = pos_vec.last().unwrap().clone();
            }
            'R' => {
